...
function CardFormHotel(props) {
    const getHotel = props.getHotel
    const url = props.url
    ...
    const [formLinkSito, setFormLinkSito] = useState("")
    ...
    const handleInputChangeLinkSito = (e) => {
        const { name, value } = e.target
        setFormLinkSito(value)
        console.log(value)
    }
    ...
    const handleSubmit = (e) => {
        ...
        postHotel(formNome, formDescrizione, formImmagine, formLinkSito, formLinkMappa, formPosizionamento, formLingua)
    ...
    const postHotel = async (nome, descrizione, immagine, linksito, linkmappa, posizionamento, lingua) => {
        try {

            const token = sessionStorage.getItem('token');
            await fetch(`${url}hotel/`, {
                method: 'POST',
                headers: {
    ...
    return (
        <form onSubmit={handleSubmit} className=" shadow-lg shadow-slate-500/60 flex flex-col w-[350px] mb-10 bg-[rgba(51,135,184,0.8)] p-3 rounded-lg gap-5">
            {/*
            <div className="flex ">
                <label className=' border-2 border-sky-900 text-xl text-white bg-sky-600 rounded-md p-1 font-medium '>Informazioni Hotel</label>
            </div>
    */}
            <Snackbar
                open={open}
            ...
            <div className="flex flex-col">
                <label className=' text-lg font-medium text-gray-800'>Link Sito</label>
                <input className=" border-2 focus:outline-none focus:border-indigo-500 rounded-md h-9" type="text" name="nome" value={formLinkSito} onChange={handleInputChangeLinkSito} />
            </div>
            <div className="flex flex-col">
            ...
            <button type="submit" className=" transition-all duration-300 flex-auto focus:outline-none focus:ring bg-sky-900 hover:bg-sky-700 active:bg-blue-800 text-white font-bold py-1 px-2 rounded">Crea</button>
