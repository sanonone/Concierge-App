...
function CardFormRistoranti(props) {
    const getRistoranti = props.getRistoranti

    const [formNome, setFormNome] = useState("")
    const [formDescrizione, setFormDescrizione] = useState("")
    const [formImmagine, setFormImmagine] = useState("")
    const [formLinkMappa, setFormLinkMappa] = useState("")
    const [formLinkMenu, setFormLinkMenu] = useState("")
    ...
    const handleInputChangeLinkMenu = (e) => {
        const { name, value } = e.target
        setFormLinkMenu(value)
        console.log(value)
    }
    ...
    const handleSubmit = (e) => {
        ...
        //handleUpload()
        postMenu(formNome, formDescrizione, formImmagine, formLinkMappa, formLinkMenu, formPosizionamento, formLingua)
    ...
    const postMenu = async (nome, descrizione, immagine, linkmappa, linkmenu, posizionamento, lingua) => {
       

        try {

            const token = sessionStorage.getItem('token');
            await fetch(`${url}ristoranti/`, {
                method: 'POST',
                headers: {
    ...
    return (
        <form onSubmit={handleSubmit} className=" shadow-lg shadow-slate-500/60 flex flex-col w-[350px] mb-10 bg-[rgba(51,135,184,0.8)] p-3 rounded-lg gap-5">
            ...
            */}
            ...
            <div className="flex flex-col">
                <label className=' text-lg font-medium text-gray-800'>Link Menù</label>
                <input className=" border-2 focus:outline-none focus:border-indigo-500 rounded-md h-9" type="text" name="nome" value={formLinkMenu} onChange={handleInputChangeLinkMenu} />
            </div>
            <div className="flex flex-col">
...
export default CardFormRistoranti